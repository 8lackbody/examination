...
public class BaseReader {
    ...
    private int GetInventoryData(byte ComAddr, int cmd, List<ReadTag> mlist) {
        byte[] buffer;
        int Count = 0;
        byte[] btArray = new byte[2000];
        int btLength = 0;
                    ...
                    while ((daw.length - index) > 5) {
                        ...
                        if (((daw[index] & 255) >= 5) && (daw[index + 1] == ComAddr) && ((daw[index + 2] & 255) == cmd)) {
                            ...
                            if (CheckCRC(epcArr, epcArr.length)) {
                                ...
                                if (logswitch == 1) Log.d("error=", "帧头错误");
                                index++;
                            }
                        } else {
                            if (logswitch == 1) Log.d("error=", "CRC错误");
                            index++;
                        }
                    }
                    if (daw.length > index) {
    ...
    public int Inventory_G2(byte ComAddr, byte QValue, byte Session, byte AdrTID, byte LenTID, byte Target,
                            byte Ant, byte Scantime, List<ReadTag> mtag) {
        byte[] buffer = null;
        if (LenTID == 0) {
            buffer = new byte[10];
            buffer[0] = (byte) 0x09;
        ...
        getCRC(buffer, buffer[0] - 1);
        SendCMD(buffer);
        return GetInventoryData(ComAddr, 0x01, mtag);
    }
